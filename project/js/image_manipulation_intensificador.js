function manipulateImage_intensificador(file) {                       
    if(file != null) {
        sendFile_intensificador(file);
    }
    else alert("Image missing!");
}

function sendFile_intensificador(file) {
	
    var fator = document.getElementById("diff_intensificador").valueAsNumber;
    ...
    if( fator == NaN || fator < 0 || fator > 255 || fator == "" || fator == undefined) {
        ...
        xhr.onreadystatechange = () => {                                            //faz o .js esperar que o POST termine e o servidor envie uma resposta antes de continuar
            ...
            if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
                ...
                show_manipulated_image_intensificador(JSON.parse( xhr.response ));       //o JSON.parse é essencial para usar a resposta como dicionário, como é pretendido
                }
            };
...
function show_manipulated_image_intensificador(response) {     //mostra a imagem  manipulada; resultado das 2 outros imagens
    ...
    if (response["result"] != "intensificador_falhou") {
        ...
        alert("The chosen image could not have its intensity altered.");
    }
...
function im_inputs_intensificador() {
    ...
    textoDiff.min = "0";
    textoDiff.max = "60";
    textoDiff.id = "diff_intensificador";
    textoDiff.style="color: blue";

    var labelFator = document.createElement("label");
    labelFator.innerHTML = "Please provide your desired intensity factor.";
...
export {manipulateImage_intensificador, sendFile_intensificador, show_manipulated_image_intensificador, im_inputs_intensificador};